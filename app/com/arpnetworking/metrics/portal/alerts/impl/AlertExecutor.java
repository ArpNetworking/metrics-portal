...
public class AlertExecutor extends AbstractPersistentActorWithTimers {
     ...
     * @param queryExecutor QueryExecutor to run the queries
    ...
    public Receive createReceive() {
        return receiveBuilder()
                .match(InstantiateAlert.class, this::instantiateAlert)
                .match(RefreshAlert.class, refresh -> loadAlert())
                .match(ExecuteAlert.class, this::executeAlert)
                ...
                .match(NotificationActor.ShuttingDown.class, this::executeShuttingDown)
                .match(Terminated.class, this::executeChildTerminated)
                .match(FlushNotificationActor.class, this::executeFlushNotificationActor)
                .build();
    }
    ...
    private void processTimeSeriesResult(final TimeSeriesResult series) {
        ...
        series.getQueryResult().getQueries().stream()
                .map(MetricsQueryResult.Query::getResults)
                .flatMap(ImmutableList::stream)
                .map(MetricsQueryResult.Result::getAlerts)
                .flatMap(ImmutableList::stream)
    ...
    private void loadAlert() {
        ...
        if (alertOptional.isPresent()) {
            ...
            if (!alert.getCheckInterval().equals(_executePeriod)) {
                _executePeriod = alert.getCheckInterval();
    ...
    private void instantiateAlert(final InstantiateAlert message) {
        if (_alertId != null) {
            return;
        }
        persist(message, msg -> {
    ...
    private void executeAlert(final ExecuteAlert executeAlert) throws ExecutionException {
        LOGGER.debug()
    ...
    private void executeShuttingDown(final NotificationActor.ShuttingDown shuttingDown) {
    ...
    private void executeChildTerminated(final Terminated terminated) {
    ...
    private void executeFlushNotificationActor(final FlushNotificationActor flush) {
