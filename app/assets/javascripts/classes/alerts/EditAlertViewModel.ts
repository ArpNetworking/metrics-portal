...
class EditAlertViewModel {
    ...
    query = ko.observable<string>("").extend({ rateLimit: { timeout: 500, method: "notifyWhenChangesStop" } });
    checkInterval = ko.observable<string>("PT1M");
    comment = ko.observable<string>("");
    container: HTMLElement;
    queryErrors = ko.observableArray<string>();
    queryWarnings = ko.observableArray<string>();
    dateRange = ko.observable<moment.FromTo>({ from: moment().subtract(2, 'hours'), to: moment()});
    formattedDateRange = ko.computed(() => {
        let range = this.dateRange();
        let start = moment(range.from);
        let end = moment(range.to);
    ...
    private executeQuery(query: string): any {
        $.ajax({
            ...
            dataType: "json",
            data: JSON.stringify({'query': query, 'start': moment(this.dateRange().from).toISOString(), 'end': moment(this.dateRange().to).toISOString()}),
